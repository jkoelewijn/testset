...
\usepackage[disabled,section]{easy-todo}
...
\setlist[enumerate,1]{label=\arabic{subsection}\alph*., ref=\arabic{subsection}\alph*}
...
Note that planning on the transit network is the current scope of the MMRI project, so the testrunner will ignore any walk legs encountered in the result.
...
\\
All tests must succesfully reply with a valid travel advice. The definition of a ``valid travel advice'' is impossible to formalize. Section~\ref{sec:nonunit} describes tests that explore the quality of travel advices.
\\
...
\item \textit{The journey planner is capable use changes within the travel advice, which may lead to a different travel advice.}
...
\item \textit{The journey planner is capable of excluding routes from travel advice.}
...
\item \textit{The journey planner is capable of excluding trips from travel advice.}
...
\item \textit{The journey planner is capable of excluding stop places from travel advice.}
...
\item \textit{The journey planner is capable of excluding stops from travel advice.}
...
\subsubsection*{Network description and Timetable}
The network consists of two stops, with a ten minute interval service, we present one trip.
This trip has received an service alert regarding calamities related to it. The test determines whether this alert is communicated inside the travel advice itself.
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \node[] (6) [left=1cm of 1a] {\tiny alert};
...
\subsection{Pre-- and posttransit transportation} \todo{correct translation of ``voor-- en navervoer''?}
...
% \newpage
% \item \textit{The journey planner uses blockcode trip-to-trip transfers in its advice.}
% \subsubsection*{Network description and Timetable}
