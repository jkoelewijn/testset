...
A blockcode is assigned to a vehicle which operates on a predefined sequential pattern of trip.
An earlier delay within the pattern must be propagated to later trips, but it also guarantees that a alighting and boarding between the last stop of the current trip and the first stop of the next trip is not required.
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \path[every node/.style={font=\sffamily\small}]
    (1) edge node {$b=1, t=1$} (2)
    (2) edge [draw=blue,densely dotted] node {$b=1, t=2$} (3);
\end{tikzpicture}
